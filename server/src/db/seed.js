...
const cleanup = transaction(() => {
  db.exec('DELETE FROM interaction_todos');
  db.exec('DELETE FROM interactions');
  db.exec('DELETE FROM person_tags');
...
const insertPerson = db.prepare(`INSERT INTO persons (name, phone, wechat, email, birthday, city, company, position, intimacy, resource_desc, need_desc, avatar_url, last_interacted_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
...
const tags = db.prepare('SELECT id, name FROM tags ORDER BY sort_order').all();

const insertPersonTag = db.prepare(`INSERT OR IGNORE INTO person_tags (person_id, tag_id) VALUES (?, ?)`);
...
for (let i = 0; i < persons.length; i++) {
  ...
  for (let j = 0; j < tagCount; j++) {
    ...
    if (!usedTags.has(tag.id)) {
      usedTags.add(tag.id);
      insertPersonTag.run(persons[i].id, tag.id);
      tagRelationCount++;
    }
  }
}
...
const insertRelationship = db.prepare(`INSERT OR IGNORE INTO relationships (person_a_id, person_b_id, relation_type, strength, introduced_by) VALUES (?, ?, ?, ?, ?)`);
let relationshipCount = 0;
const addedRelations = new Set();

function addRelation(aIdx, bIdx, type, strength, introducedBy = null) {
  ...
  insertRelationship.run(a.id, b.id, type, strength, introducedBy);
  relationshipCount++;
}

for (let i = 0; i < 6; i++) {
  addRelation(i, i + 1, '同学', randomInt(3, 5));
}
for (let i = 6; i < 14; i++) {
  addRelation(i, i + 1, '同事', randomInt(2, 4));
}
addRelation(0, 14, '夫妻', 5);
addRelation(3, 15, '亲属', 4);
addRelation(14, 16, '亲属', 3, persons[0].id);
addRelation(16, 17, '朋友', 4);

for (let i = 0; i < 10; i++) {
  const a = randomInt(0, persons.length - 1);
  let b = randomInt(0, persons.length - 1);
  while (b === a) b = randomInt(0, persons.length - 1);
  addRelation(a, b, '朋友', randomInt(2, 5));
}
...
for (let i = 0; i < 8; i++) {
  ...
  let b = randomInt(0, persons.length - 1);
  while (b === a) b = randomInt(0, persons.length - 1);
  addRelation(a, b, '合作伙伴', randomInt(3, 5));
}
...
for (let i = 0; i < 6; i++) {
  ...
  let b = randomInt(0, persons.length - 1);
  while (b === a) b = randomInt(0, persons.length - 1);
  addRelation(a, b, '客户', randomInt(2, 4));
}
...
for (let i = 0; i < 5; i++) {
  ...
  addRelation(a, b, '引荐', randomInt(1, 3), persons[intro].id);
}
...
while (relationshipCount < 42) {
  ...
  let b = randomInt(0, persons.length - 1);
  while (b === a) b = randomInt(0, persons.length - 1);
  addRelation(a, b, random(RELATION_TYPES), randomInt(1, 5));
}
console.log(`[relationships] 插入 ${relationshipCount} 条关系`);

const insertInteraction = db.prepare(`INSERT INTO interactions (person_id, interaction_date, method, content) VALUES (?, ?, ?, ?)`);
const insertTodo = db.prepare(`INSERT INTO interaction_todos (interaction_id, content, is_completed, completed_at) VALUES (?, ?, ?, ?)`);
...
for (let i = 0; i < 24; i++) {
  ...
  const result = insertInteraction.run(person.id, dateStr, method, content);
  interactionCount++;
  ...
  if (interactionsWithTodos.has(i)) {
    ...
    for (let t = 0; t < todoNum; t++) {
      ...
      insertTodo.run(interactionId, random(TODO_CONTENTS), isCompleted, completedAt);
      todoCount++;
    }
  }
}
