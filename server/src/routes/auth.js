...
router.get('/status', (req, res) => {
  const user = queryOne('SELECT id, username FROM users LIMIT 1');
...
router.post('/login', (req, res) => {
  ...
  const user = queryOne('SELECT * FROM users WHERE username = ?', [String(username)]);
...
router.get('/me', authMiddleware, (req, res) => {
  const user = queryOne('SELECT id, username, created_at FROM users WHERE id = ?', [req.user.id]);
  if (!user) {
...
router.post('/change-password', authMiddleware, (req, res) => {
  ...
  require('../db').run('UPDATE users SET password_hash = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [hash, req.user.id]);
